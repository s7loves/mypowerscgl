...
namespace Ebada.Kcgl {
    ...
    public partial class UCPopupSelectorBase : XtraUserControl {
        ...
        string getFilterString() {
            ...
            if (!string.IsNullOrEmpty(filter)) str += " and " + filter;
            return str;
        }
        string filter;
        ...
        internal void SelectRow(string displayField, object EditValue) {
            if (EditValue == null || EditValue.ToString() == "") return;
            textEdit1.Text = "";
            DataRow[] rows = DataSource.Select(string.Format("{0}='{1}'", displayField, EditValue));
            if (rows.Length > 0) {
                gridView1.FocusedRowHandle = DataSource.Rows.IndexOf(rows[0]);
                gridControl1.Update();
                gridView1.Focus();
            }
        }
