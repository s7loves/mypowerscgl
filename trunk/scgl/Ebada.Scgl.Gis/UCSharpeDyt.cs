...
namespace Ebada.Scgl.Gis {
    ...
    public partial class UCSharpeDyt : UserControl, IUCLayer {
        ...
        void treeList1_BeforeExpand(object sender, BeforeExpandEventArgs e) {
            ...
            if (id.Length==6&& e.Node.Nodes.Count==1) {
                e.Node.Nodes.Clear();
                inittq(id);
            }
            e.CanExpand = true;
        ...
        private void initxl(string pid) {
            if (pid == "all") {
                ...
                foreach (PS_xl xl in list) {
                    mTable.Rows.Add(hide, "0", xl.LineName, xl.LineCode, xl.OrgCode, "0");
                }
            }
        }
        
        private void inittq(string xlcode) {
            treeList1.BeginInit();
            IList<PS_tq> tqlist = ClientHelper.PlatformSqlMap.GetList<PS_tq>(string.Format("where left(tqcode,6)='{0}'", xlcode));
            foreach (PS_tq tq in tqlist) {
                mTable.Rows.Add(hide, "0", tq.tqName, tq.tqCode, xlcode, "1");
            }
            treeList1.EndInit();
        }
        ...
        void 台区网络图_Click(object sender, EventArgs e) {
            ...
            buildTqMap(contextMenu.Tag.ToString());

        }

        private void buildTqMap(string p) {
            setWaitMsg("生成“" + gettqname(p) + "”信息");
            showLayer(p, true);
            buildTitleInfoTQ(p);
            setWaitMsg(null);
        }
        private void buildTitleInfoTQ(string xlcode) {
            ...
            if (!rect.IsEmpty) {
                ...
                string name = gettqname(xlcode);
                string title = "" + name + "低压台区网络图 - " + DateTime.Now.Year + "年";
        ...
        private string gettqname(string xlcode) {
            string name = xlcode;
