    ...
    public class ExportLCFXWTJZGLS
    {
        ...
        public void ExportExcel(ExcelAccess ex, IList<PJ_lcfxwtjzgls> datalist)
        {
            ...
            for (int j = 0; j < datalist.Count; j++)
            {
                ...
                ex.SetCellValue(datalist[j].jhwcsj.ToString("yyyy年MM月dd日"), row + j % rowcount, col + 3);

                ex.SetCellValue(datalist[j].lszgsj.ToString("yyyy年MM月dd日"), row + j % rowcount, col + 4);
