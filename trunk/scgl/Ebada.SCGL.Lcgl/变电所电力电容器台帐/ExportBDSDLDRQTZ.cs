...
namespace Ebada.Scgl.Lcgl
{
    ...
    public class ExportBDSDLDRQTZ
    {
        ...
        public  void ExportExcel(IList<PJ_bdsdldrqtz> datalist)
        {
            //lgm
            ExcelAccess ex = new ExcelAccess();
            SaveFileDialog saveFileDialog1 = new SaveFileDialog();
            string fname = Application.StartupPath + "\\00记录模板\\变电所电力电容器台帐.xls";
            ex.Open(fname);
        ...
        public void ExportExcel(ExcelAccess ex ,IList<PJ_bdsdldrqtz> datalist)
        {
            //此处写填充内容代码
            int row = 5;
            int col = 1;
            ...
            for (int j = 0; j < datalist.Count; j++)
            {
                ...
                ex.SetCellValue(datalist[j].OrgName, row + j % rowcount, col + 1);
                ex.SetCellValue(datalist[j].zzdz, row + j % rowcount, col + 2);
