...
{
    public partial class UCWGCJCKGL : UserControl
    {
        public UCWGCJCKGL()
        {
            InitializeComponent();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (pnumli.Count == 0)
                    clccktemp.num = "CJCK" + DateTime.Now.ToString("yyyyMMdd") + string.Format("{0:D4}", 1);
                else
                {
                    clccktemp.num = "CJCK" + (Convert.ToDecimal(pnumli[0].num.Replace("CJCK", "")) + 1);
            ...
            else
            {
                if (pnumli.Count > 0)
                {
                    ...
                    if ((Convert.ToDecimal(clccktemp.num.Replace("CJCK", ""))) - (Convert.ToDecimal(pnumli[0].num.Replace("CJCK", ""))) > 1)
                    {
                        clccktemp.num = "CJCK" + (Convert.ToDecimal(pnumli[0].num.Replace("CJCK", "") + 1));
                    }
                }
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                decimal num = 0;
                 pnumli = Client.ClientHelper.PlatformSqlMap.GetListByWhere
                    ...
                    {
                        num =  (Convert.ToDecimal(pnumli[0].num.Replace("CJCK", "")));
                ...
                frmWGCJCLCKXZShow frmshow = new frmWGCJCLCKXZShow();
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            frmWGCJCLCKSingleXZ frm = new frmWGCJCLCKSingleXZ();
            ...
            <PJ_wgclcrkd>(" where  id like '" + DateTime.Now.ToString("yyyyMMdd") + "%' and type='" + comboBoxEdit6.Text + "' order by id desc ");
            if (pnumli.Count == 0)
                ((PJ_wgclcrkd)frm.RowData).num = "CJCK" + DateTime.Now.ToString("yyyyMMdd") + string.Format("{0:D4}", 1);
            else
            {
                ((PJ_wgclcrkd)frm.RowData).num = "CJCK" + (Convert.ToDecimal(pnumli[0].num.Replace("CJCK",""))+1 );
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                System.Collections.IList mclist = ClientHelper.PlatformSqlMap.GetList("SelectOneInt",
                    "select  sum(cast(kcsl as float) )  from PJ_wgclcrkd where (type = '撤旧材料入库单' or type = '撤旧材料出库单原始库存')"
