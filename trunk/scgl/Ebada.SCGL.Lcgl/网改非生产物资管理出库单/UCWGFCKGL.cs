...
{
    public partial class UCWGFCKGL : UserControl
    {
        public UCWGFCKGL()
        {
            InitializeComponent();
        ...
        {
            frmWGFCLCKXZ frm = new frmWGFCLCKXZ();
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                frmWGFCLCKXZShow frmshow = new frmWGFCLCKXZShow();
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            frmWGFCLCKSingleXZ frm = new frmWGFCLCKSingleXZ();
            frm.RowData = new PJ_wgclcrkd();
            ...
            <PJ_wgclcrkd>(" where  id like '" + DateTime.Now.ToString("yyyyMMdd") + "%' and type='" + comboBoxEdit6.Text + "' order by id desc ");
            if (pnumli.Count == 0)
                ((PJ_wgclcrkd)frm.RowData).num = "FSCCK" + DateTime.Now.ToString("yyyyMMdd") + string.Format("{0:D4}", 1);
            else
            {
                ((PJ_wgclcrkd)frm.RowData).num = "FSCCK" + (Convert.ToDecimal(pnumli[0].num.Replace("FSCCK", "")) + 1);
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                System.Collections.IList mclist = ClientHelper.PlatformSqlMap.GetList("SelectOneInt",
                    "select  sum(cast(kcsl as float) )  from PJ_wgclcrkd where (type = '非生产物资入库单' or type = '非生产物资入库单单原始库存')"
