...
namespace Ebada.Scgl.Yxgl {
    ...
    public class Export16  {
        ...
        public static void ExportExcel(PJ_16 obj)
        {
            ...
            //分支线的数目和长度
            sumgt = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_gtRowCount", "where ParentID='" + obj.LineCode + "'and LineType in('2','3')").ToString();
            ...
            string ts = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqbyqRowCount", "Where byqVol='6KV'AND byqPhase='单相'and tqID in (select tqID from ps_tq where xlCode ='" + obj.LineCode + "'or xlCode2 in(select lineid from ps_xl where ParentID='" + obj.LineCode + "'))").ToString();
            ...
            //避雷器
            string blq = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='避雷器' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(blq, 14,13);
            blq = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='避雷器'and sbModle='氧化锌硅胶' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(blq, 15, 13);
            blq = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='避雷器'and sbModle='阀型' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(blq, 16, 13);
            blq = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='避雷器'and sbModle='氧化锌硅胶带熔断器' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(blq, 17, 13);
            //绝缘子
            string jyuz = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='绝缘子' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(jyuz, 21, 13);
            jyuz = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='绝缘子'and sbModle='XP-7C' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(jyuz, 22, 13);
            jyuz = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='绝缘子'and sbModle='P-15T' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(jyuz, 23, 13);
            jyuz = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='绝缘子'and sbModle='FSW6-10/70' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
            ex.SetCellValue(jyuz, 24, 13);
            jyuz = Client.ClientHelper.PlatformSqlMap.GetObject("GetPS_tqsbRowCount", "Where sbType='绝缘子'and sbModle='FPQ2W-10/3T20' and gtID in(select gtID from ps_gt WHERE LineCode IN (SELECT lineid from ps_xl where lineid='" + obj.LineCode + "'or ParentID='" + obj.LineCode + "')").ToString();
