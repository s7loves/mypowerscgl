        ...
        public static DataTable GetMainUserControlByCaption(string mainUserCtrlCaption)
        {
            try
            {
                ...
                if (mainUserCtrlCaption!="")
                    tmpStr = " where CtrlSize='目录' or ParentID not in (select LPID from LP_Temple where 1=1 and  CtrlSize!='目录') and  CellName like '%" + mainUserCtrlCaption + "%' order by CellName";
                else
                    tmpStr = " where CtrlSize='目录' or ParentID not in (select LPID from LP_Temple where 1=1 and  CtrlSize!='目录') order by CtrlSize desc, SortID,cellname";
