...
namespace Ebada.SCGL.WFlow.Tool
{
    ...
    public class Link
    {
        ...
        public Link(BaseComponent StartTask,BaseComponent EndTask)
        {
            ...
            DllClassName = "Ebada.SCGL.WFlow.Tool.fmLinkConfig";
            selectedAnchor=-1;
        ...
        public bool isContains(Point thePoint,Point start,Point end)
        {
            ...
            if(region.IsVisible(thePoint.X,thePoint.Y))
            {
                return true;
            }
            else
        ...
        public void SaveInsertLink()
        {
            ...
            try
            {
                ...
                workflowLink.LinkId = linkGuid;
                workflowLink.WorkflowId = flowGuid;
                workflowLink.StartTaskId = startTask.TaskId;
                workflowLink.EndTaskId = endTask.TaskId;
                workflowLink.breakPointX = breakPointX;
                workflowLink.breakPointY = breakPointY;
                workflowLink.Des=Des;
                workflowLink.Condition=Condition;
                workflowLink.CommandName=CommandName;
                workflowLink.Priority=Priority;
                workflowLink.InsertLink();
            }
            catch (Exception ex)
            {
                throw ex;
