    ...
    public class WorkPlace : System.Windows.Forms.UserControl {
            ...
            for (int i = 0; i < this.TaskItems.Count; i++) {
                ...
                switch (bc.TaskType) {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        bitmap = new Bitmap(Image.FromStream(typeof(BaseComponent).Assembly.GetManifestResourceStream("Ebada.SCGL.WFlow.Tool.Resources.并行节点.ico")));
                        break;
                    default:
                        bitmap = new Bitmap(Image.FromStream(typeof(BaseComponent).Assembly.GetManifestResourceStream("Ebada.SCGL.WFlow.Tool.Resources.查看节点.ico")));
                        break;
        ...
        public void closeFlow() {
            ...
            if (!this.IsModify)//流程没有改变,不用保存直接关闭流程
            {
                ...
                if (result == DialogResult.OK) {/**/////保存流程并关闭流程            
                    SaveWorkFlow();
                    this.clearAndMenu();
                } else if (result == DialogResult.Cancel) {//不保存,关闭流程
                    this.clearAndMenu();
