...
namespace Ebada.Scgl.Yxgl {
    ...
    /// </summary>
    [ToolboxItem(false)]
    public partial class UCSD_jyzylcl : DevExpress.XtraEditors.XtraUserControl {
        private GridViewOperation<sdjls_jyzylcl> gridViewOperation;

        public event SendDataEventHandler<sdjls_jyzylcl> FocusedRowChanged;
        ...
        public UCSD_jyzylcl()
        {
            InitializeComponent();
            initImageList();
            gridViewOperation = new GridViewOperation<sdjls_jyzylcl>(gridControl1, gridView1, barManager1, new frm_jyzylcl());
            gridViewOperation.BeforeAdd += new ObjectOperationEventHandler<sdjls_jyzylcl>(gridViewOperation_BeforeAdd);
            gridViewOperation.CreatingObjectEvent += gridViewOperation_CreatingObjectEvent;
            gridViewOperation.BeforeDelete += new ObjectOperationEventHandler<sdjls_jyzylcl>(gridViewOperation_BeforeDelete);
            gridView1.FocusedRowChanged += gridView1_FocusedRowChanged;
        }

        void gridViewOperation_BeforeDelete(object render, ObjectOperationEventArgs<sdjls_jyzylcl> e) {

        }

        void gridViewOperation_BeforeAdd(object render, ObjectOperationEventArgs<sdjls_jyzylcl> e) {
            if (parentID == null)
                e.Cancel = true;
        }
        ...
        void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e) {
            if (FocusedRowChanged != null)
                FocusedRowChanged(gridView1, gridView1.GetFocusedRow() as sdjls_jyzylcl);
        }
        ...
        /// </summary>
        [Browsable(false)]
        public GridViewOperation<sdjls_jyzylcl> GridViewOperation {
        ...
        void gridViewOperation_CreatingObjectEvent(sdjls_jyzylcl newobj) {
        ...
        private void btView_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            {
                IList<sdjls_jyzylcl> jyzList = new List<sdjls_jyzylcl>();
                for (int i = 0; i < gridView1.RowCount; i++)
                {
                    var row = gridView1.GetRow(gridView1.GetVisibleRowHandle(i));
                    if (row is sdjls_jyzylcl)
                    {
