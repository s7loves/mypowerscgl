...
namespace Ebada.Scgl.Lcgl
{
    ...
    public class ExportGDSRKEdit
    {
        ...
        public void ExportGDSRKDExcel(string orgCode, string jingbanren, DateTime enterTime)
        {
            ...
            string fname = Application.StartupPath + "\\00记录模板\\供电所材料入库单.xls";
            ex.Open(fname);
            ...
            IList<PJ_gdscrk> datalist = ClientHelper.PlatformSqlMap.GetList<PJ_gdscrk>(" where OrgCode='" + orgCode + "' and type!='出库' and indate >= '" + enterTime + "' order by wpmc,indate");
            ...
            for (int j = 0; j < datalist.Count; j++)
            {
                ...
                ex.SetCellValue(datalist[j].indate.ToString(), row + j % rowcount, col + 5);
                ex.SetCellValue(datalist[j].ly, row + j % rowcount, col + 6);
        ...
        public void ExportOne(PJ_gdscrk obj)
        {
            ...
            ex.SetCellValue(obj.wpsl, row, col + 4);
            ex.SetCellValue(obj.indate.ToString(), row, col + 5);
            ex.SetCellValue(obj.ly, row, col + 6);
