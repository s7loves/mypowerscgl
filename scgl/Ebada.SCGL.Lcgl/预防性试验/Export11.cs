...
namespace Ebada.Scgl.Lcgl
{
    ...
    public class Export11 {
        ...
        public static void setSpanExcel(ExcelAccess ex, PJ_yfsyjl data, int i, int istart, int jmax, int jstart, ref int spanadd, int imax2, string sheetname,string[] sname,int itemp)
        {
            ...
            ex.SetCellValue(data.preExpTime.Year.ToString(), istart + ((i + spanadd) % jmax ) * imax2, jstart + 7);
            ex.SetCellValue(data.preExpTime.Month.ToString(), istart + ((i + spanadd) % jmax ) * imax2, jstart + 8);
            ex.SetCellValue(data.preExpTime.Day.ToString(), istart + ((i + spanadd) % jmax ) * imax2, jstart + 9);

            ex.SetCellValue(data.planExpTime.Year.ToString(), istart + ((i + spanadd) % jmax) * imax2, jstart + 10);
            ex.SetCellValue(data.planExpTime.Month.ToString(), istart + ((i + spanadd) % jmax ) * imax2, jstart + 11);
            ex.SetCellValue(data.planExpTime.Day.ToString(), istart + ((i + spanadd) % jmax ) * imax2, jstart + 12);
