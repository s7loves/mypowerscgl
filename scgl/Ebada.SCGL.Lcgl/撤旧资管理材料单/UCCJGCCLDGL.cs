...
{
    public partial class UCCJGCCLDGL : UserControl
    {
        public UCCJGCCLDGL()
        {
            InitializeComponent();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            IList<PJ_clcrkd> pnumli = Client.ClientHelper.PlatformSqlMap.GetListByWhere
                       <PJ_clcrkd>(" where  id like '" + DateTime.Now.ToString("yyyyMMdd") + "%' and type='" + comboBoxEdit6.Text + "'  order by id desc  ");
            if (pnumli.Count == 0)
                frm.strNum = "CJCLDCK" + DateTime.Now.ToString("yyyyMMdd") + string.Format("{0:D4}", 1);
            else
            {
                frm.strNum = "CJCLDCK" + (Convert.ToDecimal(pnumli[0].num.Replace("CJCLDCK", "")) + 1);
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (PJ_clcrkd pc in datalist)
                {
                    ...
                    //ClientHelper.PlatformSqlMap.Create<PJ_clcrkd>(CLDCKd);

                    ckdatalist.Add(ckd);
                    if (cktemp<1) break;
                    //num=(num + 1);
                }
                frmCJGCCLDShow frmshow = new frmCJGCCLDShow();
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            <PJ_clcrkd>(" where  id like '" + DateTime.Now.ToString("yyyyMMdd") + "%' and type='" + comboBoxEdit6.Text + "' order by id desc ");
            if (pnumli.Count == 0)
                ((PJ_clcrkd)frm.RowData).num = "CJCLDCK" + DateTime.Now.ToString("yyyyMMdd") + string.Format("{0:D4}", 1);
            else
            {
                ((PJ_clcrkd)frm.RowData).num = "CJCLDCK" + (Convert.ToDecimal(pnumli[0].num.Replace("CJCLDCK","")) + 1);
