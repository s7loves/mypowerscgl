...
namespace Ebada.Scgl.Lcgl
{
    ...
    public class ExportKHDLDRQTZ
    {
        ...
        public  void ExportExcel(IList<PJ_khdldrqtz> datalist)
        {
            //lgm
        ...
        public void ExportExcel(ExcelAccess ex ,IList<PJ_khdldrqtz> datalist)
        {
            //此处写填充内容代码
            int row = 6;
            int col = 1;
            ...
            for (int j = 0; j < datalist.Count; j++)
            {
                ...
                ex.SetCellValue(datalist[j].zhm, row + j % rowcount, col + 1);
                ex.SetCellValue(datalist[j].hm, row + j % rowcount, col + 2);
                ex.SetCellValue(datalist[j].pdrl, row + j % rowcount, col + 3);

                ex.SetCellValue(datalist[j].drqModle, row + j % rowcount, col + 4);
                ex.SetCellValue(datalist[j].sbFactory, row + j % rowcount, col + 5);
                ex.SetCellValue(datalist[j].sbnum, row + j % rowcount, col + 6);
                ex.SetCellValue(datalist[j].sbsumFactory, row + j % rowcount, col + 7);
