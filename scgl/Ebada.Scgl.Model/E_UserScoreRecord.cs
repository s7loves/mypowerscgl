...
{
    /// <summary>
    ///[E_UserScoreRecord]业务实体类
    ///对应表名:E_UserScoreRecord
    /// </summary>
    [Serializable]
    public class E_UserScoreRecord
    {
        ...
        public string Flag
        {
            ...
            {			
                if(value==null)return;
                if( value.ToString().Length > 50)
                throw new Exception("[收入/支出]长度不能大于50!");
        ...
        /// 属性名称：Reason
        /// 属性描述：事由
        /// 字段信息：[Reason],nvarchar
        /// </summary>
        [DisplayNameAttribute("事由")]
        public string Reason
        {
            ...
            {			
                if(value==null)return;
                if( value.ToString().Length > 200)
                throw new Exception("[事由]长度不能大于200!");
                if (_reason as object == null || !_reason.Equals(value))
                {
                    _reason = value;
                }
            }			 
