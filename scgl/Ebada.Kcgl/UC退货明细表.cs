...
namespace Ebada.Kcgl {
    ...
    public partial class UC退货明细表 : DevExpress.XtraEditors.XtraUserControl {
        ...
        public UC退货明细表() {
            ...
            gridViewOperation.BeforeAdd += new ObjectOperationEventHandler<Ebada.Kcgl.Model.kc_退货明细表>(gridViewOperation_BeforeAdd);
        ...
        void gridView1_CellValueChanging(object sender, CellValueChangedEventArgs e) {
            var obj = gridView1.GetRow(e.RowHandle) as kc_退货明细表;
            if (obj == null) return;
            if (e.Column.FieldName == kc_退货明细表.f_数量) {
                obj.总计 = Convert.ToDouble(e.Value) * Convert.ToDouble(obj.单价);
            } else if (e.Column.FieldName == kc_退货明细表.f_单价) {
                obj.总计 = Convert.ToDouble(e.Value) * Convert.ToDouble(obj.数量);
            }
        }

        void gridViewOperation_BeforeAdd(object render, ObjectOperationEventArgs<Ebada.Kcgl.Model.kc_退货明细表> e) {
        ...
        public void InitColumns() {
            ...
            //需要隐藏列时在这写代码
            setColumnVisible(false, kc_退货明细表.f_退货厂家, kc_退货明细表.f_工程类别, kc_退货明细表.f_材料名称_ID);
            gridView1.Columns[kc_退货明细表.f_总计].OptionsColumn.AllowEdit = false;
            gridView1.Columns[kc_退货明细表.f_规格及型号].OptionsColumn.AllowEdit = false;
            gridView1.Columns[kc_退货明细表.f_计量单位].OptionsColumn.AllowEdit = false;

            gridView1.Columns[kc_退货明细表.f_工程类别_ID].ColumnEdit = getLookup<kc_工程类别>(kc_工程类别.f_ID, kc_工程类别.f_工程类别);
            gridView1.Columns[kc_退货明细表.f_工程类别_ID].ColumnEdit.EditValueChanging += new ChangingEventHandler(工程类别ColumnEdit_EditValueChanging);
            gridView1.Columns[kc_退货明细表.f_材料名称].ColumnEdit = getclSeach();
            gridView1.Columns[kc_退货明细表.f_退货厂家_ID].ColumnEdit = getLookup<kc_供货厂家>(kc_供货厂家.f_ID, kc_供货厂家.f_厂家名称);
            gridView1.Columns[kc_退货明细表.f_退货厂家_ID].ColumnEdit.EditValueChanging += new ChangingEventHandler(供货厂家ColumnEdit_EditValueChanging);
        }
        ...
        void edit_Click(object sender, EventArgs e) {
            ...
            if (dlg.ShowDialog() == DialogResult.OK) {
                if (dlg.Selected材料 != null) {
                    var row = gridView1.GetFocusedRow() as kc_退货明细表;
        ...
        void gridViewOperation_CreatingObjectEvent(Model.kc_退货明细表 newobj) {
            newobj.退货单_ID = ParentObj.ID;
            newobj.退货日期 = ParentObj.退货时间;
        }
        /// <summary>
        /// 父表ID
        /// </summary>
        ...
        public string ParentID {
            ...
            set {
                ...
                if (!string.IsNullOrEmpty(value)) {
                    RefreshData(" where 退货单_ID='" + value + "'");
                } else {
                    RefreshData(" where 1>0");
                }
            }
        }
        Model.kc_退货单 parentObj;
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public Model.kc_退货单 ParentObj {
